...
Rules:
1. Leave ALL code blocks EXACTLY as-is (```dockerfile, ```bash, ```yaml, ```python, ```mermaid, etc.)
2. Leave docker commands, function names, variables, imports as-is: docker build, docker run, docker compose, nvidia-smi, FROM, RUN, CMD, torch, transformers, etc.
3. Keep these anglicisms in English: API, GPU, CPU, RAM, SQL, Python, PyTorch, TensorFlow, HuggingFace, LangChain, Docker, Docker Compose, Kubernetes, Git, JSON, XML, HTML, CSS, JavaScript, TypeScript, LLM, GPT, BERT, MLOps, DevOps, CI/CD, REST, NoSQL, CUDA, cuDNN, Jupyter, Pylance, RAG, vector database, Qdrant, flash-attn, bitsandbytes, NVIDIA Container Toolkit, Colab
