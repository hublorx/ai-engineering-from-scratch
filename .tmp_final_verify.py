"""Final verification of all 4 translations"""
import sys
...
1. DIAKRYTYKI: zjqebany→zjebany, Huełałem→Hulałem, pisującego→piszącego, przylapać→przyłapać, pamietam→pamiętam, Cie→Cię, Jerkałem→Jęrkałem, wdrzężaj→wdrążaj

2. NIEPOLSKIE ZNAKI: Cyrylica/rosyjskie/chińskie, "takже"→"także", "обично"→"zwykle"

3. BRAK PRZECINKA przed: że, bo, żeby, i (dwa niezależne zdania), co, który/która/które, a (kontrast), więc, aż, zanim, gdy, albo, lub, jak (porównanie)

4. ANGLICYZMY poza lista dozwolonych: DOZWOLONE to API, GPU, CPU, RAM, SQL, Python, PyTorch, TensorFlow, HuggingFace, LangChain, Docker, Docker Compose, Kubernetes, Git, JSON, XML, HTML, CSS, JavaScript, TypeScript, LLM, GPT, BERT, MLOps, DevOps, CI/CD, REST, NoSQL, CUDA, Jupyter, JupyterLab, Colab, Pylance, Black, Ruff, Debugpy, GitLens, SSH, VS Code, GPU, Qdrant, RAG, itp.

5. KOD W TŁUMACZENIU: bloki kodu NIE MIALY byc tlumaczone. Sprawdz czy kod jest nie ruszony.
...
Raport:"""
    ...
    try:
        ...
        report_path = translated_path.replace("_translated.md", "_final_verification.txt")
